...
@router.post(
    ...
    description="Create a new citizen account. No authentication required. Returns user and tokens.",
...
@router.post(
    ...
    response_model=AuthResponse,
    summary="Login",
    description="Login with **phone + password** (citizens) or **user_id + password** (staff/admin). Provide exactly one of phone or user_id. Optional department UUID restricts login to that department. No auth required. For staff (fieldManager, fieldAssistant), the response includes **worker_profile** with department_id, ward_display, and department (name/short_code) so clients can filter workers and grievances.",
    response_description="User and access/refresh tokens.",
)
...
@router.post(
    ...
    description="Exchange a valid refresh token for new access and refresh tokens. No Bearer auth required; send refresh_token in body.",
...
@router.get(
    ...
    description="Return the authenticated user with full details. Staff get **worker_profile** (department_id, ward_display, department) loaded. **Access:** any authenticated user (Bearer token required).",
...
@router.get(
    ...
    description="Return full user details by user ID. **Access:** same user (own profile) or admin/fieldManager.",
